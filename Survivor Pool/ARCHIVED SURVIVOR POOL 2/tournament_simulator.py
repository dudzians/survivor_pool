...
class TournamentSimulator:
    ...
    def simulate_game(self, game_id: str, team1: str, team2: str, round_num: int) -> GameResult:
        ...
        try:
            ...
            # Normalize winner and loser team names
    ...
    def get_actual_teams(self, team_placeholder: str) -> str:
        ...
        try:
            ...
            return normalize_team_name(winner)
        except Exception as e:
    ...
    def simulate_round(self, games):
        ...
        for _, game in games.iterrows():
            ...
            if team_a.startswith('Winner of:'):
                game_id = team_a.split(': ')[1]
                if game_id in self.results:
                    team_a = self.results[game_id].winner
            if team_b.startswith('Winner of:'):
                game_id = team_b.split(': ')[1]
                if game_id in self.results:
                    team_b = self.results[game_id].winner
    ...
    def simulate_tournament(self):
        ...
        # Group games by day and simulate each round
        for day, day_games in schedule.groupby('day'):
            print(f"Simulating games for {day}")
            results = self.simulate_round(day_games)
            
            # Update team seeds for winners
            for result in results:
                winner_seed = self.get_team_seed(result.winner)
                if winner_seed:
                    self.team_seeds[f'Winner of: {result.game_id}'] = winner_seed
    ...
    def get_teams_playing_on_date(self, target_date):
        ...
        day_games = self.schedule_df[self.schedule_df['day'].dt.date == target_date.date()]
        ...
        for _, game in day_games.iterrows():
            ...
            # Add normalized team names to the set
            if team_a:
                teams.add(normalize_team_name(team_a))
            if team_b:
                teams.add(normalize_team_name(team_b))
        ...
        for _, game in day_games.iterrows():
            ...
            if game_id in self.results:
                ...
                if winner.startswith('Winner of:'):
                    # For Winner of: games, we need to get the actual winner from the previous game
                    prev_game_id = winner.split(': ')[1]
                    if prev_game_id in self.results:
                        winner = self.results[prev_game_id].winner
                    else:
                        print(f"Warning: No result found for previous game {prev_game_id}")
                        continue
                winners.add(normalize_team_name(winner))
