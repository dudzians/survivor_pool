...
def load_schedule(file_path: str) -> pd.DataFrame:
    ...
    for date in sorted(df['day'].unique()):
        games_on_date = df[df['day'].dt.date == date.date()]
        print(f"{date.strftime('%Y-%m-%d')}: {len(games_on_date)} games")
...
def get_games_for_date(schedule: pd.DataFrame, target_date: str) -> pd.DataFrame:
    ...
    try:
        ...
        games = schedule[schedule['day'].dt.date == target_date.date()]
...
def get_teams_playing_on_date(schedule: pd.DataFrame, target_date: str) -> set:
    ...
    # Debug logging
    print(f"Looking for games on {target_date}")
    print("Available dates:")
    for date in sorted(schedule['day'].unique()):
        print(f"  {date}")
    ...
    # For Sweet 16 and later rounds, we need to handle "Winner of" games
    teams = set()
    for _, game in games.iterrows():
        if isinstance(game['teamA'], str):
            teams.add(game['teamA'].strip())
        if isinstance(game['teamB'], str):
            teams.add(game['teamB'].strip())
